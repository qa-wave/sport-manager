 ...
 * adding a new dependency. The math below is the standard "offset probe"
 * trick: ask the engine what the wall-clock time is for a given UTC instant
 * in a specific zone, then invert.
 */
 ...
 *
 * Inputs:
 *   date: Date representing a calendar day (we only use its y/m/d)
 *   hh:  hour 0-23 (local to tz)
 *   mm:  minute 0-59
 *   tz:  IANA timezone (e.g. "Europe/Prague")
 *
 * Algorithm:
 *   1. Construct a naive UTC timestamp from (y,m,d,hh,mm) as if it were UTC.
 *   2. Find the offset that tz had at that approximate instant.
 *   3. Adjust once. For DST "spring forward" / "fall back" days the offset
 *      before and after the adjustment can differ; re-probe and correct.
 *
 * This handles the 2x/year DST transitions: if 17:00 local happens twice
 * (fall back), we pick the first occurrence deterministically; if it doesn't
 * exist at all (spring forward 02:00 → 03:00), we snap to the valid hour.
 */
export function zonedWallTimeToUtc(
  ...
  // Shift: if local=UTC+offset, then UTC=local-offset.
 ...
 * The returned Date is just a carrier for year/month/day; time-of-day is
...
export function* eachLocalDay(
  ...
  // Compute the first local calendar day >= fromUtc and iterate day-by-day
  // in UTC until we pass toUtc. We step by 24h; DST shifts are absorbed in
  // `zonedWallTimeToUtc` when we render the actual timestamp.
  ...
  // Convert current (y,m,d) to a UTC midnight for iteration bookkeeping.
  ...
  while (currentUtc <= endUtc) {
    ...
    // Keep the "carrier" date's UTC components in sync with the local date
    // if a DST transition shifted which UTC day the local midnight falls on.
    y = nextLocal.y;
    m = nextLocal.m;
    d = nextLocal.d;
