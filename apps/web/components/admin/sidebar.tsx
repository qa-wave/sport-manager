...
export function Sidebar({
    ...
    return (
      <Link
        ...
        className={cn(
          ...
          active
            ? 'bg-primary/15 font-medium text-chrome-foreground'
            : 'text-chrome-muted hover:bg-white/[0.06] hover:text-chrome-foreground'
        )}
      >
        {active && (
    ...
    NAV_GROUPS.map((group) => {
      ...
      return (
        <div key={group.key}>
          {group.key !== 'main' && (
            <>
              <Separator className="my-2 bg-chrome-border" />
              <div className="px-3 py-1 text-[10px] font-semibold uppercase tracking-widest text-chrome-muted/70">
                {group.label}
              </div>
            </>
          )}
  ...
  return (
    <TooltipProvider delayDuration={0}>
      ...
      <aside
        className={cn(
          'hidden flex-col border-r border-chrome-border bg-chrome text-chrome-foreground transition-[width] duration-200 ease-in-out md:flex',
          'relative',
          collapsed ? 'w-14' : 'w-56'
        )}
      >
        {/* Brand */}
        <div className="relative flex h-14 items-center gap-2.5 border-b border-chrome-border px-3">
          ...
          {!collapsed && (
            <div className="flex flex-col">
              ...
              <span className="text-[11px] uppercase tracking-wider text-chrome-muted">
                {roleLabel}
              </span>
            </div>
          )}
        ...
        <div className="relative flex items-center justify-between px-3 py-2.5">
          <Button
            variant="ghost"
            size="icon"
            className="h-7 w-7 text-chrome-muted hover:text-chrome-foreground"
            onClick={onToggle}
          >
            {collapsed ? (
          ...
          {!collapsed && (
            <button
              data-tour="help"
              className="flex h-6 w-6 items-center justify-center rounded-full border border-chrome-border text-[11px] font-bold text-chrome-muted transition-colors hover:border-primary/60 hover:text-primary"
              title="Nápověda"
              aria-label="Nápověda"
            >
              ?
      ...
      <div className="md:hidden">
        ...
        <aside
          className={cn(
            'fixed bottom-0 left-0 top-0 z-50 flex w-64 flex-col border-r border-chrome-border bg-chrome text-chrome-foreground transition-transform duration-300 ease-in-out',
            mobileOpen ? 'translate-x-0' : '-translate-x-full'
          )}
        >
          {/* Brand + close button */}
          <div className="relative flex h-14 items-center justify-between border-b border-chrome-border px-3">
