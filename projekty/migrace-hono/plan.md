...
- Zachovat typovou bezpečnost (Zod kontrakty z `@sport-manager/contracts` zůstávají single source of truth)
...
- Prisma client sdílen z `@sport-manager/db` — stejný package, žádná změna
- Zod validace zůstává identická — importuje z `@sport-manager/contracts`
...
- Sdílení Prisma clienta mezi Server Components a Hono route — singleton pattern v `@sport-manager/db`
...
- [ ] Portovat Zod validace ze stávajících DTO na `@sport-manager/contracts` schémata — backend-vyvojar
...
2. Restartovat NestJS proces (`pnpm --filter @sport-manager/api dev`)
...
OTÁZKY:
  ...
  2. Sdílet Prisma client instanci mezi Hono routes a Next.js Server Components přes singleton v `@sport-manager/db`?
